...
import { Plus, Minus, RefreshCw, Check, X, Edit2, Trash2 } from "lucide-react";
import { CounterQueryProvider, useCounterAtom } from "@/providers/CounterContextQueryProvider";
import { TodoQueryProvider, useTodoAtom, type Todo } from "@/providers/TodoContextQueryProvider";
import { ThemeQueryProvider, useThemeAtom } from "@/providers/ThemeContextQueryProvider";
...
function CounterExampleContent() {
  ...
  const increment = () => {
    setCounter((prev: { value: number; name: string }) => ({ ...prev, value: prev.value + 1 }));
  };
  ...
  const reset = () => {
    setCounter((prev: { value: number; name: string }) => ({ ...prev, value: 0 }));
  };
  ...
  return (
    ...
    >
      ...
      <div className="bg-gray-100 dark:bg-gray-900 rounded-lg p-4">
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">{t('currentState')}</p>
    ...
    >
      ...
      <div className="space-y-6 mb-6">
        ...
        <div>
          ...
          <div className="flex gap-2">
            <button
              onClick={() => setTheme((prev: typeof theme) => ({ ...prev, mode: 'light' }))}
            ...
            <button
              onClick={() => setTheme((prev: typeof theme) => ({ ...prev, mode: 'dark' }))}
              className={`px-4 py-2 rounded-lg transition-colors ${
                theme.mode === 'dark'
                  ? 'bg-blue-500 text-white'
                  : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300'
        ...
        <div>
          ...
          <div className="flex gap-2">
            {colors.map((color) => (
              <button
                key={color}
                onClick={() => setTheme((prev: typeof theme) => ({ ...prev, primaryColor: color }))}
        ...
        <div>
          ...
          <div className="flex gap-2">
            {(['small', 'medium', 'large'] as const).map((size) => (
              <button
                key={size}
                onClick={() => setTheme((prev: typeof theme) => ({ ...prev, fontSize: size }))}
      ...
      <div 
        className={`p-6 rounded-lg border-2 ${
          theme.mode === 'dark' ? 'bg-gray-900 border-gray-700' : 'bg-white border-gray-200'
        }`}
      >
        <h3 
      ...
      <div className="bg-gray-100 dark:bg-gray-900 rounded-lg p-4 mt-6">
        ...
        <pre className="text-xs overflow-x-auto">
          <code className="text-gray-800 dark:text-gray-200">{JSON.stringify(theme, null, 2)}</code>
        </pre>
      </div>
    </motion.div>
  );
